...
One question to consider is whether to maintain a backup strategy.  Early testers recommended the historical path, flying the one-person capsule, then the two, then moving on to the three-person spacecraft for lunar missions.  They would then maintain Gemini/Voskhod as a backup program, and switch over to it if their primary suffered a catastrophic failure.  This can be a very handy strategy, though it’s only practical if you are planning to go to the Moon with Apollo/Soyuz or XMS-2/Lapot.  It is also more costly than going straight from Mercury/Vostok to what you plan to fly to the Moon.

The game has many keyboard commands; those that activate buttons usually have the corresponding letter displayed in a different color.  The game also inclues a help feature activated by pressing F1.
...
New to RIS is Advanced Preferences.  The game doesn’t offer a fancy graphical screen for it, just a plain text file.  The file can be in different locations, but is usually found in a user’s Documents\raceintospace folder (Windows) or ~/.raceintospace folder (Linux).  Windows users may find it in an alternative location such as C:\Users\*username*\AppData\Local\VirtualStore\Program Files (x86)\raceintospace.  Alternatively, it may be located in the folder where the game resides on your hard drive (e.g., Program Files on XP, Program Files (x86) on Windows 10, /usr/games on Ubuntu).  Wherever you find it, it will be named simply "`config`" (with no extension).  Open the config file with a plain text editor such as Notepad or gedit.
...
In BARIS, this was all or nothing.  If a component was at or above 75%, it would give the full tech transfer; if it wasn’t quite there, it would give nothing at all.  RIS (as of v.1.1) now gives partial credit when your hardware is somewhere above starting Safety.
...
The "Visit  R&D Facility" button is a shortcut to your Research building.  You can’t just start, say, a capsule program and start using it; you first have to invest time and money to make it spaceworthy.
...
* Kicker-A:  Used to push a Gemini/Voskhod or XMS-2/Lapot, or an unaccompanied LM, out of orbit toward the Moon.  Can push only one major component (capsule/shuttle or lunar lander), so can be used on LOR missions but not EOR, nor for the Historical Lunar Landing or single-launch lunar orbital LM test.
* Kicker-B: Used to push a Gemini/Voskhod or XMS-2/Lapot, and/or an unaccompanied LM, out of orbit toward the Moon.  Can push two major components (capsule/shuttle and lunar lander), so can be used on LOR, EOR, or Historical missions.
...
In the recruitment screen, you will see the number of available positions, with each selected candidate’s name and skills.  LM, EVA, and Docking won’t be known until individuals begin their training, so those skills won’t show here.  Candidate skills are the following:
...
This is where to go for intelligence briefings.  Each fall you’ll receive a report about your opponent's activities: developing a given component, preparing a given mission, etc.  Again, this is often inaccurate, but it's one tool to help guess at your opponent's progress.
...
When they leave Basic Training, they go to the Astronaut Complex (US) or Cosmonaut Center (USSR).
...
LM Points are not specific to the lander they were earned with.  That is, if for some reason you have purchased both landers (as above) and gained 3 LM points with one lander, you could use the other for the lunar landing with no LM penalty.

Direct ascent landings (Jupiter/LK-700) and Soyuz Lunar Landing (using the C-Kicker) do not require LM points, and in fact it is not possible to gain them using that hardware.
...
This is a basic Earth-orbital satellite, and can be used only on the Orbital Satellite mission.  It is necessary to develop this unless you'll be flying direct ascent, since Explorer/Sputnik Safety is rolled against when a docking module powers on.
...
This is a three-person capsule, capable of all docking and lunar missions, and can accomplish any Duration level.  It does not need a kicker to go to the Moon, and does not require docking modules to dock with LMs.
...
Kennedy’s challenge of landing men on the Moon and returning safely to the Earth is the ultimate goal of winning the space race.  It is especially challenging, when starting with a meager budget and [a] fledgling space program.
