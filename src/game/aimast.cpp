...
// *** check status ***
    ...
    if (Data->P[plr].AIStat <= 2) {
        ...
        Data->P[plr].Buy[0][0] = 0;
        Data->P[plr].Buy[1][0] = 0;
...
//reassing player level
    ...
    case 6:
        if (Data->P[plr].Rocket[1].Num <= Data->P[plr].Manned[1].Num) {
            ...
            if (Level_Check != 0) {
                Data->P[plr].Buy[1][1] = 0;
                Data->P[plr].Buy[2][1] = 0;
                RDafford(plr, 2, 1);
                RDafford(plr, 1, 1);
            }
        } else {
