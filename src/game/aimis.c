    ...
    case 3:
        if (i < 4) {
            ...
            if (Data->P[plr].Misc[4].Num < 0) {
                cost += Data->P[plr].Misc[4].InitCost;
            }

            if (Data->P[plr].Misc[4].Num == 0) {
                cost += Data->P[plr].Misc[4].UnitCost;
            }
        }
...
void GetHard(char plr)
{
    ...
    for (i = 0; i < 7; i++)
        if (rck == -1 && bwgt[i] > prwt[Data->P[plr].WList[0].Asp]) {
            rck = i;
        }
...
char BoostCheck(char plr)
{
    ...
    if (Data->P[plr].Rocket[4].Num >= 2 && Data->P[plr].Rocket[4].Safety > 75) {
        check = 1;
    } else {
        if (Data->P[plr].Rocket[4].Num == 2) {
            RDafford(plr, 1, 4);
        } else {
            check = GenPur(plr, 1, 4);
        }
...
void Scheduler(char plr)
{
    ...
    if (Mis.Jt == 1) {
        ...
        for (i = 0; i < 2; i++)
            if (Data->P[plr].Future[i].MissionCode == 0 && Data->P[plr].LaunchFacility[i] == 1 &&
                Data->P[plr].Future[i + 1].MissionCode == 0 && Data->P[plr].LaunchFacility[i + 1] == 1) {
                prim = i;
            }

        if (prim != -1) {
            tmoon[0][0] = Data->P[plr].WList[0].Asp;
            tmoon[0][1] = Data->P[plr].WList[0].Asp;
            tmoon[1][0] = Mis.Days;
            tmoon[1][1] = Mis.Days;
    ...
    for (i = 0; i < 3; i++) if (Data->P[plr].Future[i].MissionCode == 0) {
            AIFuture(plr, 1, i, 0);
        }
