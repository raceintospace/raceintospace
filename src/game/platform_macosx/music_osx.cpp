...
struct disposable_track {
    disposable_track* next;
    MusicPlayer player;
    time_t dispose_after;
};
disposable_track* disposable_tracks;
...
void music_load(enum music_track track)
{
    ...
    for (int i = 0; music_key[i].track != M_MAX_MUSIC; i++) {
    ...
    FILE* midi_f = sOpen(name, "rb", FT_MIDI);
    size_t fileLength;
    if (midi_f != NULL) {
    ...
    if (midi_data == NULL) {
        LOG_WARNING("could not read MIDI file `%s'", name);
        music_files[track].unplayable = 1;
        return;
    }
    
    // Initialize the sequence
    OSStatus rv = NewMusicSequence(&music_files[track].sequence);
    if (rv == 0) {
        ...
        rv = MusicSequenceLoadSMFDataWithFlags(music_files[track].sequence, 
                                               CFDataCreate(NULL, (UInt8*)midi_data, fileLength), 
                                               kMusicSequenceLoadSMF_PreserveTracks);
#else  /* not deprecated, but requires 10.5 or later */
        rv = MusicSequenceFileLoadData(music_files[track].sequence, 
                                       CFDataCreate(NULL, (UInt8*)midi_data, fileLength), 
    ...
    // Did it work?
    if (rv != 0) {
        LOG_WARNING("could not understand MIDI file `%s'", name);
        music_files[track].unplayable = 1;        
    } else {
...
fail:
    LOG_WARNING("unexpected error playing MIDI track; result=%ld", result);
    music_files[track].unplayable = 1;
    return;    
}
...
void music_pump()
{
    ...
    if (disposable_tracks != nullptr && disposable_tracks->dispose_after < time(NULL)) {
        // Stop and free
        ...
        disposable_track* next = disposable_tracks->next;
    ...
    for (int i = 0; i < M_MAX_MUSIC; i ++) {    
        ...
        if (! music_files[i].playing) continue;
        if (! music_files[i].loop) continue;
