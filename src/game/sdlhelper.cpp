    ...
    for (j = 0; j < ARRAY_LENGTH(ranges); ++j)
        for (i = ranges[j].start; i < ranges[j].end; ++i) {
            if (!fade_info.force_black) {
                pal_colors[i].r = display::graphics.legacyScreen()->pal()[3 * i] * 4;
                pal_colors[i].g = display::graphics.legacyScreen()->pal()[3 * i + 1] * 4;
                pal_colors[i].b = display::graphics.legacyScreen()->pal()[3 * i + 2] * 4;
            } else {
                pal_colors[i].r = 0;
                pal_colors[i].g = 0;
                pal_colors[i].b = 0;
