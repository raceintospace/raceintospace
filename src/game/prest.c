...
char Was_Goal(char total, char which)
{
    ...
    while (i != 0x7f) {                                // not branched
        if (Mev[i].loc == val && Mev[i].StepInfo < 3000) {
            x = minn(1, x);
        }

        if ((which == ORBSAT || which == LUNFLY || which == PROBELAND) && Mev[i].StepInfo > 500) {
            x = -1;
        }

        if (Mev[i].StepInfo >= 3000) {
            x = -1;    // Manned will only be affected
        }
    ...
    switch (which) {
    case EORBIT:
        return Was_Goal(total, MANSPACE);

    case LPASS:
        return Was_Goal(total, EORBIT);

    case LORBIT:
        return Was_Goal(total, LPASS);

    case LLAND:
        return Was_Goal(total, LORBIT);

    case EWALK:
        return Was_Goal(total, LWALK);

    default:
        return -1;  // no goal step achieved
    }
}
#endif /* DEAD_CODE */
